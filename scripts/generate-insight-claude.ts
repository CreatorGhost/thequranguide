/**
 * Generates surah insight using Claude via `claude -p` (headless mode).
 * Uses your Claude Code subscription — no separate API key needed.
 *
 * Usage:
 *   npx tsx scripts/generate-insight-claude.ts 1 18 93
 *
 * Requires context files from fetch-context.ts to exist in scripts/output/
 * Must be run OUTSIDE of Claude Code (not nested).
 */
...
const MODEL = "sonnet"; // sonnet for balance of quality + speed; use "opus" for max quality

// ─── System Prompt (same as GPT script, tuned for Claude) ───
...
CONNECTIONS:
- Don't just state "both surahs mention X." Explain the thematic arc: how one surah's message flows into or contrasts with another's.
...
- coreMessage.emotionalArc: 40-70 words
- coreMessage.buildUp: 50-100 words
...
interface SurahInsight {
  ...
  }[];                        // ALL stories/parables. Empty array if none.
  connections: {
    surah: number;
    surahName: string;
    relationship: string;
  ...
  notableVerses: {
    ...
    relatedHadith: string;    // MANDATORY
  }[];                        // 3-4 notable verses
...
- Write with warmth, depth, and personal engagement — make the reader feel addressed
...
function callClaude(prompt: string): string {
  ...
  const tmpFile = join(OUTDIR, "_claude_prompt_tmp.txt");
  writeFileSync(tmpFile, prompt);

  try {
    const result = execSync(
      `cat "${tmpFile}" | claude -p --output-format json --model ${MODEL} --allowedTools ""`,
      {
        encoding: "utf-8",
...
// ─── Validate output ───
function validateOutput(parsed: any, surahNum: number, ayahCount: number): string[] {
  const issues: string[] = [];
  ...
  if (wc(parsed.historicalContext?.background) < 60)
    issues.push(`background too short (${wc(parsed.historicalContext?.background)}w)`);
  if (wc(parsed.coreMessage?.thesis) < 35)
    issues.push(`thesis too short (${wc(parsed.coreMessage?.thesis)}w)`);

  for (const s of parsed.sections || []) {
    if (wc(s.summary) < 50) issues.push(`Section "${s.title}" too thin (${wc(s.summary)}w)`);
  }
...
function processSurah(surahNum: number): void {
  ...
  console.log(`\nGenerating insight for Surah ${surahNum} (${ctx.englishName}) via Claude ${MODEL}...`);

  const fullPrompt = `${SYSTEM_PROMPT}\n\n---\n\n${buildUserPrompt(ctx)}`;
  const inputEst = Math.ceil(fullPrompt.length / 4);
  console.log(`   Estimated input: ~${inputEst} tokens`);

  const startTime = Date.now();

  let rawResult: string;
  try {
    rawResult = callClaude(fullPrompt);
  } catch (err: any) {
  ...
  // Parse the claude -p JSON output wrapper
  let content: string;
  try {
    const cliOutput = JSON.parse(rawResult);
    content = cliOutput.result || rawResult;
  } catch {
    content = rawResult;
  }

  // Extract JSON from the content (may be wrapped in code fences)
  ...
  try {
    ...
    const rawPath = join(OUTDIR, `insight-${surahNum}-claude-failed.txt`);
    writeFileSync(rawPath, content);
    console.log(`   Failed to parse JSON — raw saved to ${rawPath}`);
    return;
  }

  // Validate
  const issues = validateOutput(parsed, surahNum, ctx.numberOfAyahs);
  if (issues.length > 0) {
  ...
  // Save
  const outPath = join(OUTDIR, `insight-${surahNum}-claude.json`);
  ...
  const wc = (s: string) => (s ? s.split(/\s+/).filter(Boolean).length : 0);
  const total = [
    parsed.historicalContext?.background,
    parsed.historicalContext?.occasionOfRevelation,
    parsed.audience,
    parsed.coreMessage?.thesis,
    parsed.coreMessage?.emotionalArc,
    parsed.coreMessage?.buildUp,
    ...(parsed.sections || []).map((s: any) => s.summary),
    ...(parsed.keyVocabulary || []).map((w: any) => w.deeperMeaning),
    ...(parsed.stories || []).map((s: any) => `${s.narrative} ${s.lesson}`),
    ...(parsed.notableVerses || []).map(
      (v: any) => `${v.tafsirExcerpt} ${v.linguisticNote} ${v.practicalApplication} ${v.relatedHadith}`
    ),
    ...(parsed.connections || []).map((c: any) => c.relationship),
    parsed.scholarlyNotes,
  ]
    .filter(Boolean)
...
if (args.length === 0) {
  console.log("Usage: npx tsx scripts/generate-insight-claude.ts 1 18 93");
  console.log("");
  console.log("IMPORTANT: Run this OUTSIDE of Claude Code (not nested).");
  console.log("Context files must exist — run fetch-context.ts first.");
  process.exit(1);
}

console.log(`Generating insights via Claude ${MODEL} for surahs: ${args.join(", ")}`);
console.log(`NOTE: Processing sequentially (one at a time) to respect rate limits.\n`);

for (const surahNum of args) {
  processSurah(surahNum);
}
